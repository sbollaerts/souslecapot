...
// Labo 6 — observabilité. Portée « scoped » : une collection de traces par
// session (circuit Blazor). La politique de sécurité publie ses événements vers
// l'observabilité, qui les rattache à la trace courante (couplage faible).
builder.Services.AddScoped<ObservabilityService>();
builder.Services.AddScoped(sp => new SecurityPolicy
{
    EventSink = sp.GetRequiredService<ObservabilityService>().RecordSecurityEvent,
});
