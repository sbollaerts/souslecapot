...
public class RagService
{
    ...
    private List<(string Document, string Heading, string Content)> LoadChunks()
    {
        ...
        // Labo 5 : on indexe TOUS les documents Markdown (hors README et questions
        // de test), y compris la fixture document-injection-test.md — c'est elle qui
        // permet de démontrer l'injection indirecte via le RAG.
        var excluded = new HashSet<string> { "README-corpus.md", "questions-test.md" };
        foreach (var path in Directory.GetFiles(_corpusDir, "*.md")
