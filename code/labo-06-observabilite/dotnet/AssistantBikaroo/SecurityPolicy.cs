...
public partial class SecurityPolicy
{
    ...
    /// <summary>
    /// Labo 6 : callback optionnel (observabilité). La politique ne connaît pas le
    /// service d'observabilité — elle publie ses événements, que l'application
    /// rattache à la trace courante. Couplage faible.
    /// </summary>
    public Action<SecurityEvent>? EventSink { get; set; }

    public void Record(string type, string severity, string source, string details, string action)
    {
        var securityEvent = new SecurityEvent(type, severity, source, details, action);
        Events.Add(securityEvent);
        EventSink?.Invoke(securityEvent);
    }
