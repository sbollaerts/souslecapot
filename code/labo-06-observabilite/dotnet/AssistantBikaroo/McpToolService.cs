...
public class McpToolService
{
    ...
    // Tools d'écriture : jamais proposés au modèle dans le chat libre. Ils
    // modifient des données et ne sont appelés qu'après confirmation explicite
    // (par l'orchestration, via CallToolAsync).
    private static readonly HashSet<string> WriteTools = new() { "create_revision_request" };
    ...
    /// <summary>
    /// Appelle directement un tool MCP et renvoie son résultat (texte JSON). Utilisé
    /// par l'orchestration pour piloter le processus de façon déterministe, y compris
    /// le tool d'écriture create_revision_request après confirmation.
    /// </summary>
    public async Task<string> CallToolAsync(string name, Dictionary<string, object?> arguments)
    {
        await using var client = await McpClient.CreateAsync(CreateTransport());
    ...
    {
        await using var client = await McpClient.CreateAsync(CreateTransport());
        // Les tools d'écriture ne sont pas exposés au modèle (voir WriteTools).
        var tools = (await client.ListToolsAsync())
            .Where(t => !WriteTools.Contains(t.Name))
            .Select(ToOllamaTool)
            .ToList();
