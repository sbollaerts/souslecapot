...
# Fichiers du dossier ressources qui ne font pas partie du corpus indexé.
EXCLUDED_DOCUMENTS = {"README-corpus.md", "questions-test.md"}


def load_chunks(corpus_dir):
    """Charge les documents du corpus et les découpe en chunks.

    corpus_dir : dossier contenant les documents Markdown du corpus.

    Labo 5 : on indexe TOUS les documents Markdown (hors README et questions de
    test), y compris la fixture de sécurité document-injection-test.md — c'est
    elle qui permet de démontrer l'injection indirecte via le RAG.
    """
    chunks = []
    for path in sorted(Path(corpus_dir).glob("*.md")):
        if path.name in EXCLUDED_DOCUMENTS:
            continue
