...
public class BikarooTools
{
    ...
    // (le client ne le propose jamais au modèle dans le chat libre).
    [McpServerTool(Name = "create_revision_request")]
    [Description("Crée une demande de révision de frais pour un trajet et renvoie son identifiant et son statut.")]
    public static string CreateRevisionRequest(RevisionStore store,
        [Description("Identifiant du membre.")] string member_id,
        [Description("Identifiant du trajet.")] string trip_id,
        [Description("Description du problème.")] string description,
        [Description("Informations complémentaires collectées.")] string informations_complementaires)
        => store.Create(member_id, trip_id, description, informations_complementaires);
}
